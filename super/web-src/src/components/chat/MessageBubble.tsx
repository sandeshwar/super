...
export function MessageBubble({ message, busy, isLast }: { message: ChatMessage; busy?: boolean; isLast?: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Tool trace per assistant */}
      {message.role === 'assistant' && message.content && (
        <div style={{ marginTop: 'var(--space-2)', padding: 'var(--space-2)', background: 'var(--bg-1)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-sm)', fontSize: 'var(--text-xs)', color: 'var(--fg-3)' }}>
          <div style={{ fontWeight: 600, fontSize: 'var(--text-2xs)', letterSpacing: 'var(--tracking-wide)', marginBottom: 4 }}>TOOL TRACE</div>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <span className="mono" style={{ padding: '2px 6px', background: message.gate?.ok ? 'var(--green-bg)' : 'var(--red-bg)', border: `1px solid ${message.gate?.ok ? 'var(--green-border)' : 'var(--red-border)'}`, borderRadius: 99, color: message.gate?.ok ? 'var(--green)' : 'var(--red)' }}>{message.gate?.ok ? 'grounded' : 'unverified'}</span>
